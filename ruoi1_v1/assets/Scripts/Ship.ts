...
export default class Ship extends cc.Component {
    ...
    createBullet() {
        ...
        bullet.parent = this.node;
        var pos = this.ship.getPosition();
        bullet.setPosition(cc.v2(pos.x-25,pos.y+this.ship.height/2-20))
        //////let bullet = null;
        if (this.bulletPool.size() > 0) { // use size method to check if there're nodes available in the pool
        bullet = this.bulletPool.get();
        } else { // if not enough node in the pool, we call cc.instantiate to create node
        ...
        bullet.parent = this.node;
        var pos = this.ship.getPosition();
        bullet.setPosition(cc.v2(pos.x-15,pos.y+this.ship.height/2-10))
        ///////
        let bullet = null;
        if (this.bulletPool.size() > 0) { // use size method to check if there're nodes available in the pool
        bullet = this.bulletPool.get();
        ...
        bullet.parent = this.node;
        var pos = this.ship.getPosition();
        bullet.setPosition(cc.v2(pos.x,pos.y+this.ship.height/2+10))
                   let bullet = null;
        if (this.bulletPool.size() > 0) { // use size method to check if there're nodes available in the pool
        bullet = this.bulletPool.get();
        } else { // if not enough node in the pool, we call cc.instantiate to create node
        ...
        bullet.parent = this.node;
        var pos = this.ship.getPosition();
        bullet.setPosition(cc.v2(pos.x+15,pos.y+this.ship.height/2))
        //////
        let bullet = null;
        if (this.bulletPool.size() > 0) { // use size method to check if there're nodes available in the pool
        bullet = this.bulletPool.get();
        ...
        bullet.parent = this.node;
        var pos = this.ship.getPosition();
        bullet.setPosition(cc.v2(pos.x-35,pos.y+this.ship.height/2-20))
        //////let bullet = null;
        if (this.bulletPool.size() > 0) { // use size method to check if there're nodes available in the pool
        bullet = this.bulletPool.get();
        } else { // if not enough node in the pool, we call cc.instantiate to create node
        ...
        bullet.parent = this.node;
        var pos = this.ship.getPosition();
        bullet.setPosition(cc.v2(pos.x-25,pos.y+this.ship.height/2-10))
        ///////
        let bullet = null;
        if (this.bulletPool.size() > 0) { // use size method to check if there're nodes available in the pool
        bullet = this.bulletPool.get();
